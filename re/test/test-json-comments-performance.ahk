...
; to see if adding support for comments reduces performance
; Conclusion: Possible small performance drop, but not significant.
...
class test {
    static Call() {
        ...
        str := RegExReplace(FileRead('..\..\test-files\example.json'), '\s+\}\s*$', '')
        ...
        ; cache original pattern
        p := QuickParse.Pattern
        ...
        _test() {
            ...
            MsgBox(p1 '`n' p2 '`n' p3 '`n' p4 '`n' p5 '`n' p6)
        }
    }
}
