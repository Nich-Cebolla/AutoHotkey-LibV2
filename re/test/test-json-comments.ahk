...
; to verify that the pattern with support for comments works.
; it does.
...
class test {
    static Call() {
        ...
        split := StrSplit(Trim(FileRead('..\..\test-files\example.json'), '`n'), '`n')
        i := 0
        s := split[++i] '`n// comment // \\//\\`n' split[++i] '/**/`n' split[++i] '/*/*//******///`n' split[++i] '//`n//`n//`n    //`n/*`n`n*/`n'
        s .= split[++i] '`n/*eoweofwonfwn*/`n  /*`n  ojreoqijrqorj`n  eofijwefoj`n*/'
        loop split.Length - i {
            s .= split[++i] '`n'
        }
        ...
        QuickParse.Pattern := 'S)' pObject '|' pArray
...
class QuickParse {
    ...
    static Call(&Str?, Path?, Encoding?, Root?, AsMap := false, MapCaseSense := false) {
        ...
        Z(m, *) {
            OutputDebug(++commentcount ': ' StrReplace(m[0], '`n', '``n') '`n')
        }
        X(m, *) {
            OutputDebug(++commentcount ': ' StrReplace(m[0], '`n', '``n') '`n')
        }
