...
class QuickStringify {
     ...
     */
    __New(Options?) {
        options := QuickStringify.Options(Options ?? unset)
        this.Eol := options.Eol
        this.Indent := QuickStringify_IndentHelper(options.IndentLen, options.IndentChar)
    }
    /**
    ...
    class Options {
        ...
        __New(options?) {
            if IsSet(options) {
                for prop in QuickStringify.Options.Prototype.OwnProps() {
...
class QuickStringify_IndentHelper extends Array {
    static __New() {
        this.DeleteProp('__New')
        proto := this.Prototype
        proto.__IndentLen := ''
