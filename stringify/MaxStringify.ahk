...
class MaxStringify {
     ...
     */
    __New(Options?) {
        options := MaxStringify.Options(Options ?? unset)
        this.Eol := options.Eol
        this.Indent := MaxStringify_IndentHelper(options.IndentLen, options.IndentChar)
    ...
    class Options {
        ...
        __New(options?) {
            if IsSet(options) {
                for prop in MaxStringify.Options.Prototype.OwnProps() {
...
class MaxStringify_IndentHelper extends Array {
    static __New() {
        this.DeleteProp('__New')
        proto := this.Prototype
        proto.__IndentLen := ''
