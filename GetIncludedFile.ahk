...
class GetIncludedFile {
    ...
    __New(Path, Recursive := true, ScriptDir := '', AhkExeDir := '', encoding?) {
        ...
        if !drive {
            if GetIncludedFile_ResolveRelativePathRef(&Path) {
                throw ValueError('The input ``Path`` is invalid.', , Path)
            }
        }
        ...
        if Recursive {
            ...
            loop {
                ...
                ct := 0
                content := FileRead(active.fullPath, encoding ?? unset)
                pos := 1
                while RegExMatch(content, 'imS)^[ \t]*#include(?<again>again)?[ \t]+(?<i>\*i[ \t]+)?(?:<(?<lib>[^>]+)>|(?<path>.+))', &match, pos) {
        ...
        _Proc() {
            if _path := match['path'] {
                _path := Trim(StrReplace(_path, '``;', ';'), '`s`t"')
                ...
                if !drive {
                    if GetIncludedFile_ResolveRelativePathRef(&_path, cwd) {
                        throw ValueError('``GetIncludedFile`` encountered a relative path that was unable to be resolved to its full path.', , 'Path: ' _path '; relative to: ' cwd)
                    }
                }
     ...
     *
     * @param {Boolean} [CodeLinesOnly = true] - If true, the following resitrictions are applied:
     * - Lines that only have a comment are removed
     * - Lines that only contain whitespace are removed
     *
     * @returns {Integer} - The line count.
     */
    CountLines(CodeLinesOnly := true) {
        ct := 0
        if CodeLinesOnly {
            if encoding := this.encoding {
                for path in this.unique {
                    StrReplace(RegExReplace(RegExReplace(FileRead(path, encoding), 'S)(?:[\r\n]+|^)(?:[ \t]*;.*|[ \t]*/\*[\w\W]*?\*/|[ \t]+)', '`n'), '\R+', '`n'), '`n', , , &n)
                    ct += n + 1
                }
            } else {
                for path in this.unique {
                    StrReplace(RegExReplace(RegExReplace(FileRead(path), 'S)(?:[\r\n]+|^)(?:[ \t]*;.*|[ \t]*/\*[\w\W]*?\*/|[ \t]+)', '`n'), '\R+', '`n'), '`n', , , &n)
                    ct += n + 1
                }
            }
        } else {
     ...
     */
    GetUnique() {
        ; This is here for reverse compatibility with an earlier version where the "unique" property
     ...
     * were found are represented in the output string.
     *
     * Regarding the files that were **not** found, for any of the items that include the **ignore**
     * parameter ("\*i"), the `#Include` or `#IncludeAgain` statement is replaced with an empty string.
     * If there are any not-found items that do **not** include the **ignore** parameter ("\*i"), the
     * `outNotFound` variable is set with an array of those {@link GetIncludeFile.File} objects.
     *
     * @param {VarRef} [outNotFound] - If all files were accounted for, this variable is set with an
     * empty string. Otherwise, the variable that will receive an array of {@link GetIncludedFile.File}
     * objects as described by this method's description.
     *
                    ...
                    if item.skipped {
                        if item.isAgain {
                            ...
                            if _item.HasOwnProp('content') {
                                s := RegExReplace(s, '(?<=[\r\n]|^)\Q' item.match[0] '\E', _item.content, , 1)
                            } else {
                                ...
                                s := RegExReplace(s, '(?<=[\r\n]|^)\Q' item.match[0] '\E', _item.Build(), , 1)
                            }
                        } else {
                            s := RegExReplace(s, '(?<=[\r\n]|^)\Q' item.match[0] '\E', '', , 1)
                        }
                    } else if item.HasOwnProp('content') {
                        s := RegExReplace(s, '(?<=[\r\n]|^)\Q' item.match[0] '\E', item.content, , 1)
                    } else {
                        for __item in this.stack {
                            if item.fullPath = __item.fullPath {
                                throw Error('Two scripts cannot be mutual dependents with one another.', , '1: ' item.fullPath '; 2: ' __item.fullPath)
 ...
 */
GetIncludedFile_ResolveRelativePathRef(&Path, RelativeTo?) {
