...
PropsInfoObj_Method := PropsInfoObj.Get('Method')
; or
PropsInfoObj_Prop := PropsInfoObj['Prop']
...
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method.Index) ; 1

; --- B. `PropsInfoItem` objects - `InfoItem.Kind()` -----------------------------------------------
...
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method.Kind) ; Call
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Prop.Kind) ; Get

; --- C. `PropsInfoItem` objects - `InfoItem.GetValue()` -------------------------------------------
...
; Since we have the function object itself, the first parameter is exposed; an error will be thrown
; if the function is called without it. Here, we simply pass the owner object to the first parameter.
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method_Func(PropsInfoObj_Method.GetOwner())) ; Alpha return value

; Passing the owner object above was just to showcase the `PropsInfoObj.GetOwner` method and
; to contextualize `PropsInfoItem`'s design. The function itself does not refer to `this`, and so
; any value would be fine.
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method_Func('')) ; Alpha return value

; Remember that function objects also have a built-in `Name` property which can be used for various
; tasks.
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method_Func.Name) ; Alpha.Prototype.Method
...
PropsInfoObj_Method_Alt := PropsInfoObj_Method.Alt[1]
...
PropsInfoObj_Method_Alt_Func := PropsInfoObj_Method_Alt.GetFunc(, 1)
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method_Alt_Func()) ; Alpha.base return value

; This also creates a new object without a name.
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj_Method_Alt_Func.Name) ;

; The type of object returned by `Func.Prototype.Bind` is a `BoundFunc`.
OutputDebug('`n' A_LineNumber ': ' Type(PropsInfoObj_Method_Alt_Func)) ; BoundFunc
...
try {
    ...
    OutputDebug('`n' A_LineNumber ': ' err.Message) ; `PropsList` must be an array.
}
...
; Since the `PropsList` variable of `Transplantinator` is called in a 1-param `for` loop, using a
...
MyFilterFunc(PropsInfoObj) {
    if PropsInfoObj.GetValue(&Value) {
...
; We can also iterate the filters and identify them by name, as we saw earlier all functions
; except anonymous functions have a name.
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj.Filter.Has(FilterIndex)) ; 1
for Index, FilterObj in PropsInfoObj.Filter {
    if FilterObj.Name == 'MyFilterFunc' {
        PropsInfoObj.FilterDelete(FilterObj.Index)
    }
}
...
PropsInfoObj.FilterRemoveFromExclude('Capacity')
OutputDebug('`n' A_LineNumber ': ' PropsInfoObj.Filter.Exclude) ; ,Length,__New,__Init,
