...
; To use the timer, you need a function you want called every time the timer's period elapses.
Callback() {
    Duration := A_TickCount - test.LastTick
    test.g['EdtDisplay'].Text := '`r`n' FormatTime(A_Now, 'HH:mm:ss') ' :: ' Duration '`r`n' SubStr(test.g['EdtDisplay'].Text, 1, 2000)
    test.LastTick := A_TickCount
    ; If the history functionality is active, the return value is preserved in the timer object's array.
    ; If the history functionality is not active, the return value has no effect.
    return Duration
}
...
; The following is all code to make the gui work
...
class test {
    ...
    static Call() {
        ...
        g := this.g := Gui()
        width := this.width
        g.Add('Edit', 'w' width ' r20 -Wrap +HScroll vEdtDisplay')
        this.LastTick := 0
        PropsInfoObj := GetPropsInfo(t, , '__Item,Call', false)
        this.Filter := PropsInfo.FilterGroup((InfoItem) => SubStr(InfoItem.Name, 1, 2) == '__')
        PropsInfoObj.FilterSet(this.Filter)
        buttons := this.buttons := []
        props := this.props := []
        setters := this.setters := []
        this.Updater := Updater(g, props)
        for InfoItem in PropsInfoObj {
            switch InfoItem.KindIndex {
                case 1: buttons.Push(InfoItem)
                case 2: props.Push(InfoItem)
                case 3:
                    setters.Push(InfoItem)
                    props.Push(InfoItem)
                case 4: setters.Push(InfoItem)
                case 5:
                    props.Push(InfoItem)
                    if InfoItem.Name !== 'Callback' {
                        setters.Push(InfoItem)
                    }
            }
        }
        g['EdtDisplay'].GetPos(, &edty, , &edth)
        y := edty + edth + g.MarginY
        x := g.MarginX
        buttons.Push({ Name: 'ViewItems', GetOwner: (*) => '' }, { Name: 'UpdateProps', GetOwner: (*) => '' })
        for InfoItem in buttons {
            o := InfoItem.GetOwner()
            if Type(o) == 'Prototype' && o.__Class == 'Array' {
                continue
            }
            buttons[A_Index] := g.Add('Button', 'x' x ' y' y ' Section vBtn' InfoItem.Name, InfoItem.Name)
            buttons[A_Index].OnEvent('Click', HClickButtonGeneral)
            buttons[A_Index].GetPos(&btnx, , &btnw, &btnh)
            if btnx + btnw > width - g.MarginX * 2 {
                y += btnh + g.MarginY
                x := g.MarginX
                buttons[A_Index].Move(x, y)
                x += g.MarginX + btnw
            } else {
                x += g.MarginX + btnw
            }
        ...
        left := 0
        y += btnh + G.MarginY
        for InfoItem in setters {
            groups.Push({
                text: g.Add('Text', (A_Index == 1 ? 'x' g.MarginX ' y' y : 'xs') ' Section vTxt' InfoItem.Name, InfoItem.Name ': ')
              , edit: g.Add('Edit', 'ys vEdt' InfoItem.Name)
              , button: g.Add('Button', 'ys vBtn' InfoItem.Name, 'Update')
            })
            groups[-1].button.OnEvent('Click', HClickButtonUpdate)
            groups[-1].button.GetPos(&btnx, , &btnw)
            if btnx + btnw > left {
                left := btnx + btnw
            }
        }
        groups[1].button.GetPos(, &y)
        x := left + g.MarginX
        w := width - x - g.MarginX
        for InfoItem in props {
            if !InfoItem.GetValue(&Value) {
                props[A_Index] := {
                    text: g.Add('Text', (A_Index == 1 ? 'y' y ' x' x : 'xs') ' Section vTxt' InfoItem.Name '_prop', InfoItem.Name ': ')
                  , prop: g.Add('Text', 'ys w' w ' vProp' InfoItem.Name, IsObject(Value) ? '{ ' Type(Value) ' }' : this.t.%InfoItem.Name%)
                  , InfoItem: InfoItem
                }
            }
        }
        g.Show('w' (600 + g.MarginX * 2))

        HClickButtonGeneral(Ctrl, *) {
            if Ctrl.Name == 'BtnViewItems' {
                s := ''
                for item in this.t {
                    s .= '{ Start: ' item.Start ', End: ' item.End ', Result: ' item.Result ' }`r`n'
                }
                Ctrl.Gui['EdtDisplay'].Text := s SubStr(Ctrl.Gui['EdtDisplay'].Text, 1, 2000)
            } else if Ctrl.Name == 'BtnUpdateProps' {
                this.Updater()
            } else {
                name := SubStr(Ctrl.Name, 4)
                Result := this.t.%name%()
                Ctrl.Gui['EdtDisplay'].Text := (
                    '`r`nCalling: ' name '; Result: ' (IsObject(Result) ? '{ ' Type(Result) ' }' : Result || '""')
                    '`r`n' SubStr(Ctrl.Gui['EdtDisplay'].Text, 1, 2000)
                )
            }
        }
        HClickButtonUpdate(Ctrl, *) {
            name := SubStr(Ctrl.Name, 4)
            this.t.%name% := this.g['Edt' name].Text
            this.Updater()
        }
    }
}
...
class Updater {
    ...
    Call(*) {
        ...
        for obj in this.props {
            ...
            if !InfoItem.GetValue(&Value) {
                if IsObject(Value) {
                    g['Prop' InfoItem.Name].Text := '{ ' Type(Value) ' }'
                } else {
                    g['Prop' InfoItem.Name].Text := Value
                }
            }
        }
    }
...
; The following code is from here, also written by me and licensed with MIT license: https://github.com/Nich-Cebolla/AutoHotkey-LibV2/tree/main/inheritance
