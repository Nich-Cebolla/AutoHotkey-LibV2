...
class test {
    static Call() {
        g := this.g := gui()
        g.Add('Button', 'Section vBtnMake', 'Make').OnEvent('Click', HClickButtonMake)
        g.Add('Button', 'ys vBtnExit', 'Exit').OnEvent('Click', (*) => ExitApp())
        g['BtnMake'].GetPos(&x, &y, &w, &h)
        o := {
            GetButton: false
          , SetButton: false
          , startX: 10
        ...
        list := []
        for prop in ShowTooltip.DefaultOptions.OwnProps() {
            list.Push(prop)
        }
        list.Push('String')
        controls := this.Controls := MakeInputControlGroup(g, list, o)
        controls.Get('Duration').Edit.Text := 2000
        controls.Get('X').Edit.Text :=
        controls.Get('Y').Edit.Text := 0
        controls.Get('Mode').Edit.Text := 'M'
        controls.Get('Y').Edit.GetPos(&x, &y, &w, &h)
        controls.Get('String').Edit.Text := 'string'
    ...
    static Make() {
        ...
        n := st(
            controls.Get('String').Edit.Text
          , {
                Duration: controls.Get('Duration').Edit.Text
              , X: controls.Get('X').Edit.Text
              , Y: controls.Get('Y').Edit.Text
              , Mode: controls.Get('Mode').Edit.Text
            }
        )
    }
}
