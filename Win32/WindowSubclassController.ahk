/*
    Github: https://github.com/Nich-Cebolla/AutoHotkey-LibV2/blob/main/Win32/WindowSubclassController.ahk
    Author: Nich-Cebolla
    License: MIT
*/
...
class WindowSubclassController {
     ...
     *
     * @param {*} [SubclassProc = WindowSubclassController_SubclassProc] - The function that will
     * be used as the subclass procedure, or a pointer to the function.
     *
     * If `SubclassProc` is a function object, it is passed to
     * {@link https://www.autohotkey.com/docs/v2/lib/CallbackCreate.htm CallbackCreate}, passing
     * option "F" to the "Options" parameter.
     ...
     */
    __New(HwndSubclass, uIdSubclass?, SubclassProc := WindowSubclassController_SubclassProc, SuppressUniqueIdError := false) {
     ...
     *
     * Parameters:
     * 1. **{Integer}** - The WM_COMMAND code
     * 2. **{Integer}** - HwndSubclass
     * 3. **{Integer}** - uMsg
     * 4. **{Integer}** - wParam
     * 5. **{Integer}** - lParam
     * 6. **{Integer}** - uIdSubclass
     * 7. **{WindowSubclassController}** - This {@link WindowSubclassController} object
     *
     ...
     *
     * Parameters:
     * 1. **{Integer}** - HwndSubclass
     * 2. **{Integer}** - uMsg
     * 3. **{Integer}** - wParam
     * 4. **{Integer}** - lParam
     * 5. **{Integer}** - uIdSubclass
     * 6. **{WindowSubclassController}** - This {@link WindowSubclassController} object
     *
     * If the function returns a nonzero value, that value gets returned to the system and no further
     * functions are called. The effect of this depends on the message.
     *
     ...
     *
     * Parameters:
     * 1. **{WindowSubclass_Nmhdr}** - The {@link WindowSubclass_Nmhdr} object
     * 2. **{Integer}** - HwndSubclass
     * 3. **{Integer}** - uMsg
     * 4. **{Integer}** - wParam
     * 5. **{Integer}** - lParam
        ...
        if subclassController.flag_Command {
            if callbackCollection := subclassController.CommandGet((wParam >> 16) & 0xFFFF) {
                for cb in callbackCollection {
                    if result := cb((wParam >> 16) & 0xFFFF, HwndSubclass, uMsg, wParam, lParam, uIdSubclass, subclassController) {
                        return result
                    }
                }
            }
    ...
    case 0x004E: ; WM_NOTIFY
        if subclassController.flag_Notify {
            ...
            if callbackCollection := subclassController.NotifyGet(hdr.code_int) {
                for cb in callbackCollection {
                    if result := cb(hdr, HwndSubclass, uMsg, wParam, lParam, uIdSubclass, subclassController) {
                        return result
                    }
                }
            }
    ...
    default:
        if subclassController.flag_Message {
            if callbackCollection := subclassController.MessageGet(uMsg) {
                for cb in callbackCollection {
                    if result := cb(HwndSubclass, uMsg, wParam, lParam, uIdSubclass, subclassController) {
                        return result
                    }
                }
            }
